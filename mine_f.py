...
class MINE(object):
    ...
    def __init__(self, params, z, z_hat, name="MINE"):
        ...
        self.z = z
        self.z_hat = z_hat
        self.loss, self.train_step = self._build_graph(z, z_hat)
    ...
    def _build_graph(self, z, z_hat):
        ...
        with tf.name_scope("stat_net_t"):
            out_t = self._stat_net(z)
        with tf.name_scope("stat_net_t_prime"):
            out_t_prime = self._stat_net(z_hat, reuse=True)
        # tf.summary.histogram("out_t", out_t)
        # tf.summary.histogram("out_t_prime", out_t_prime)
        
        loss = self._loss_init(out_t, out_t_prime)
        
        train_step = self._optimizer_init(loss, self.p["learning_rate"])
    ...
    def _stat_net(self, inputs, reuse=False):
        ...
        with tf.variable_scope("stat_net"):
            ...
            for i in range(n_hidden):
                s = tf.layers.dense(inputs=s, units=n_units, activation=tf.nn.leaky_relu,
                                    reuse=reuse, name="s_"+str(i+1))
            out = tf.layers.dense(inputs=s, units=1, activation=None,
